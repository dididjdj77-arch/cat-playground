...
## D-057. Guard 함수 시그니처 표준 (House 구현 기준)
- 무엇:
  - guard_soft_state(p_deleted_at timestamptz, p_hidden_at timestamptz) RETURNS boolean
    - 둘 다 NULL이면 TRUE, 하나라도 NOT NULL이면 FALSE
  - guard_block(p_viewer_id uuid, p_target_user_id uuid) RETURNS boolean
    - p_viewer_id IS NULL이면 FALSE (auth-only 정책 강제, D-035)
    - blocks 테이블에 (viewer, target) 또는 (target, viewer) 존재하면 FALSE
    - 그 외 TRUE
  - guard_visibility_published(p_visibility text, p_published_at timestamptz) RETURNS boolean
    - p_visibility = 'public' AND p_published_at IS NOT NULL이면 TRUE
    - 그 외 FALSE
- 의미: Guard 함수를 RPC 외부에서 독립 테스트 가능하게 하고, House auth-only 정책(D-035)을 Guard 레벨에서 강제한다.
- 영향: 모든 공개 RPC는 이 Guard 조합을 사용한다. anon 접근은 guard_block에서 FALSE 반환으로 차단된다.
- 변경: ADR 권장.

## D-058. House RPC 404 반환 방식 (NULL 반환 + 상위 레이어 매핑)
- 무엇:
  - House 공개 RPC(rpc_get_public_house_slots_summary 등)는 조회 불가 시 NULL을 반환한다.
  - HTTP 404 매핑은 API 레이어(Edge Function 또는 클라이언트)에서 처리한다.
  - RPC 내부에서 RAISE EXCEPTION을 사용하지 않는다(v1 기준).
- 이유: Supabase RPC의 에러 핸들링 일관성 + 클라이언트 유연성 확보.
- 영향: 클라이언트는 NULL 응답을 404로 해석해야 한다. API-CONTRACTS/클라이언트 문서에 명시 필요.
- 변경: ADR 불필요(구현 세부).

## D-059. v1 room_key 하드코딩 + slot_key 검증
- 무엇:
  - v1에서 room_key는 'living_room'으로 하드코딩한다(파라미터로 받지 않음).
  - slot_key는 서버에서 허용 리스트를 검증한다: slot_01 ~ slot_08만 허용.
  - 허용 리스트 외 slot_key 요청 시 400 에러 반환.
- 허용 리스트 SSOT: RPC 내부 상수 배열 사용(v1). app_config로 이관 시 ADR로만.
- 확장 정책(D-006 유지): 슬롯 확장은 서버 허용 리스트 + 클라이언트 씬 config 동시 업데이트.
- 영향: rpc_save_house_slot(p_slot_key, p_inventory_item_id) 시그니처에 room_key 없음.
- 변경: ADR 불필요(v1 범위 제한).

## D-060. house_profiles 자동 생성 정책
- 무엇:
  - 사용자가 처음 rpc_get_my_house() 호출 시 house_profiles row가 없으면 자동 생성한다.
  - 기본값: visibility='private', published_at=NULL, hidden_at=NULL, deleted_at=NULL.
  - 자동 생성은 rpc_get_my_house 내부에서 INSERT ON CONFLICT DO NOTHING 또는 UPSERT로 처리한다.
- 이유: 가입 즉시 하우스 탭 진입 가능(빈 상태라도 에러 없이).
- 영향: house_profiles에 대한 별도 "생성" RPC 불필요.
- 변경: ADR 불필요(UX 편의).

## D-061. House 구현 실행 계획 (EP-HOUSE-IMPL)
- 무엇:
  - House 도메인 구현은 3단계 EP(Execution Packet)로 진행한다.
  - EP-1 (Foundation): 스키마 + Guard 함수 + 본인용 RPC
  - EP-2 (Public Access): 공개 조회 RPC
  - EP-3 (Integration): CI 테스트 + QA
  - 상세 계획: docs/EP-HOUSE-IMPL.md 참조.
- 의미: 각 EP를 독립 검증 가능하게 분리해 롤백/재작업 범위를 최소화한다.
- 영향: 구현은 EP-1 → EP-2 → EP-3 순서 강제. EP-N 실패 시 EP-(N-1)까지는 유지.
- 변경: ADR 불필요(프로세스).
