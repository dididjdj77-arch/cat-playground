...
## D-027 Observation 저장 H3 하이브리드 원칙
...
- 변경: 원칙 변경 또는 승격 기준 변경은 ADR 필요.

## D-028 멀티캣 관찰 저장 원자성(RPC 트랜잭션)
- 무엇: UI 1카드 = DB N레코드 구조의 멀티캣 관찰 저장은 Supabase RPC(또는 동등한 서버 트랜잭션)에서 그룹 생성 + N레코드 upsert를 단일 트랜잭션으로 처리하고 실패 시 전체 롤백한다.
- 의미: 부분 성공으로 인한 데이터 찢김(Data Tearing)을 방지한다.
- 영향:
  - UX: 실패 시 일관된 실패/재시도 경험
  - 데이터: 참조 무결성 보장, 고아 레코드 방지
  - 운영: 재처리/복구 단순화
- 변경: 트랜잭션 경계 또는 저장 모델 변경은 ADR 필요.
