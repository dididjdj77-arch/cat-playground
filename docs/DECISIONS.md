...
## D-017 작성자 탭(닉네임/아바타) 중심 탐색 UX(액션 메뉴)
- Class: GUARD
- 무엇: 작성자(닉네임/아바타) 탭 시 즉시 이동이 아니라 메뉴(고양이정보/하우스보기/냥스타그램). 작성글/활동은 추후 확장.
...
## D-099. 공개 하우스 도달 경로 v1(작성자 탭 액션 메뉴)
- Class: POLICY
- 무엇:
  - v1에서 타인 공개 하우스 진입의 표준 경로는 "작성자(닉네임/아바타) 탭 -> 액션 메뉴 -> 하우스 보기"로 고정한다(D-017).
  - `하우스 보기`는 `/u/{nickname}/house`로 이동한다(라우팅 SSOT는 `docs/ROUTES-AND-IA.md`).
  - v1은 공개 하우스 전용 탐색 피드/탭을 두지 않는다(노출 위치는 작성자 경유로 제한).
  - 공개 하우스는 auth-only 정책을 유지하며(SEO 제외), 조건 미충족 시 404 통일 규칙을 따른다(D-035).
