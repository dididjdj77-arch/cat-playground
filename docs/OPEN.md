...
## O-025 계획/알림 도메인 최소 설계
- 왜 미결정: v1 범위 미확정. 투약 계획/알림 등 미래 시점 니즈 존재.
- 결정에 필요한 질문:
  - v1에 포함할지, v1.1+ 후순위로 둘지?
  - 포함 시 최소 범위: 단발 할 일 + 단발 알림? 반복 스케줄까지?
  - 투약 계획(D-060)과의 연동 방식?
- 설계 원칙(권장):
  - 관찰(log_date=과거/오늘)과 시간축이 달라 별도 도메인 분리.
  - 확장 가능하게 최소 스키마로 시작.
- See: D-060 (투약은 인벤 제외)

## O-026 관찰 → 냥스타 CTA 흐름 상세
- 왜 미결정: UX 흐름 상세가 문서화되지 않음.
- 제안 방향:
  - 저장/도메인은 분리 (관찰=사실, 냥스타=표현).
  - 관찰 저장 후 CTA: "오늘 이야기로 공유하기 →" → Composer 이동.
  - 관찰 요약을 초안에 자동 삽입 (삭제 가능).
- 결정에 필요한 질문:
  - 자동 삽입되는 관찰 요약의 형식/범위?
  - 링크 테이블(관찰↔냥스타 FK) 도입 여부? (v1 보류 권장)
